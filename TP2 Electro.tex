...
En este primer ejercicio, se determinó la configuración de un circuito dispuesto en una caja, pudiendo ser RC serie o RC paralelo. Para hallarlo se decidió medir la continuidad entre la entrada y la salida de este. Una vez constatada, se midió el valor de la resistencia, y se concluyó que, en caso de ser un circuito RC serie, el multímetro indicaría un valor fuera de escala, en caso contrario, se obtendría el valor de dicho dispositivo. De esta forma se concluyó que la forma del circuito es el que se muestra en la figura \ref{RCserie}.

%ya que se concluyó que, en caso de no poder medirla, se estaría frente a un circuto RC serie, donde el capacitor impide determinar el parámetro anteriormente mencionado. En caso contrario, se mediría la continuidad sobre la resistencia, como se muestra en la figura \ref{RCserie}.
...
Las gráficas de las tensiones concuerdan con las de un circuito RC paralelo, como era esperable. 
...
Otro interés del trabajo fue hallar la constante de tiempo del circuito $ \tau = RC $. Para esto primero se obtuvo el valor de la resistencia $ R $ colocando un un multímetro en paralelo del circuito, obteniéndose $ R_{exp} = 223.5 \Omega $.
